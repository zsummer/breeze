...
Proto4z.WorldSceneInfoAck[2] = {name="scenes", type="SceneInfoArray" }  
...
Proto4z.WorldTryEnterSceneReq[1] = {name="type", type="ui16" } --类型, 如果是不同类型,该请求会触发场景切换动作 
...
Proto4z.WorldTryEnterSceneAck[2] = {name="scenes", type="SceneInfoArray" } --要检查状态, 如果是主城则状态会置换为INSTACING, 如果是需要匹配的 状态为匹配中 
...
Proto4z.WorldSceneInfoNotice[2] = {name="scenes", type="SceneInfoArray" } --要检查状态, 如果是主城则状态会置换为INSTACING, 如果是需要匹配的 状态为匹配中 
...
Proto4z.WorldLeaveSceneReq[1] = {name="type", type="ui16" } --类型 
...
Proto4z.WorldLeaveSceneAck[2] = {name="scenes", type="SceneInfoArray" } --要检查状态, 如果是主城则状态会置换为INSTACING, 如果是需要匹配的 状态为匹配中 
...
Proto4z.WorldQuitSceneReq[1] = {name="type", type="ui16" } --类型 
...
Proto4z.WorldQuitSceneAck[2] = {name="scenes", type="SceneInfoArray" } --要检查状态, 如果是主城则状态会置换为INSTACING, 如果是需要匹配的 状态为匹配中 
